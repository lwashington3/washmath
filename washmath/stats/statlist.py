...
class StatList(list):
    ...
    @staticmethod
    def standardDeviationSums(lst1, lst2) -> float:
        """
        Returns the sum of each standard deviation in lst1 and lst2. Used for calculating the correlation coefficient.
        :param stats.StatList.StatList lst1:
        :param stats.StatList.StatList lst2:
        :return: The sum of standard deviations
        :rtype: int or float
        """
        if len(lst1) != len(lst2):
            raise ValueError(f"Length of lst1 does not equal the length of lst2 {len(lst1)}!={len(lst2)}")
        elif not isinstance(lst1, StatList):
            raise TypeError(f"lst1 is not of type 'StatList'\nAdd StatList() around the parameter you pass")
        elif not isinstance(lst2, StatList):
            raise TypeError(f"lst2 is not of type 'StatList'\nAdd StatList() around the parameter you pass")
        summation = 0.0
        for x, y in zip(lst1, lst2):
            x_diff = x - lst1.mean
            y_diff = y - lst2.mean
            summation += (x_diff * y_diff)
        return removeDecimal(summation)
    ...
    def standard():
        """
        Creates two standard StatLists to test how the class works
        :return: Two StatList of collections used in Unit 5
        :rtype: :class, class: :stats.StatList.StatList, stats.StatList.StatList)
        """
        return StatList([2, 5, 8, 8, 10, 12]), StatList([2, 4, 7, 6, 9, 8])
...
if __name__ == "__main__":
    print(StatList([1, 2, 3, Fraction(4, 5)]).dataSummary())
